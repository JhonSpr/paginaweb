$(document).ready(function() {
    ...
    $(".btngui").click(function(e) {
        dulieu = $("#contact-form").serialize();
        $.post(base_url + "/report.html", dulieu, xuly);
    });
    ...
    $('input[name=order]').click(function() {
        ...
        $(".load_order").html(text);
        $(this).parent().parent().parent().parent().find('.tmp').removeClass('active');
        $(this).parent().parent().parent().addClass('active');
    });
    $('input[name=status]').click(function() {
        var text = $(this).attr('data-text');
        $(".load_status").html(text);
        $(this).parent().parent().parent().parent().find('.tmp').removeClass('active');
        $(this).parent().parent().parent().addClass('active');
    });
...
function xuly(data) {
    if (data == "OK") {
        ...
        $("#contact-form input:text").val('');
        $("#contact-form input:radio").prop("checked", false);
        $("#contact-form textarea").val('');
        $("#contact-form").hide();
        $("#msg_eror").html('Gracias por el informe. Revisaremos y arreglaremos este episodio lo antes posible.');
        setTimeout(function() {
            $('.mask').fadeOut();
            $('.reportForm').fadeOut();
        }, 3000);
...
function ajaxBookmarkEpisode(id, sort, keyword, url, callback) {
    $.ajax({
        dataType: 'html',
        type: 'post',
        async: false,
...
function removeBookmark(elm, id, url) {
    $.ajax({
        type: 'post',
        dataType: 'json',
        async: false,
